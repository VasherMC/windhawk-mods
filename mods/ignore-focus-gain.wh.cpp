// ==WindhawkMod==
// @id              ignore-focus-gain
// @name            Ignore Focus Gain
// @description     Make games believe they're never on top
// @version         0.0.1
// @author          Vasher
...
/*
# Ignore Focus Gain
This is an altered/opposite version of [Ignore Focus Loss](https://windhawk.net/mods/ignore-focus-loss).
Instead of making a program think it is always focused, by suppressing
focus loss events, this version suppresses focus gain events, so a program
should believe it never gets focused (ie it is always in the background).
 
This may break the program, or may have no effect whatsoever. Use at your own risk!

Disabling the mod should allow any running programs to check the current focus
and receive focus changes as normal without needing to restart it.
...
HWND WINAPI GetForegroundWindowHook(void) {
    if (g_enabledHooks) {
        // always return null (the window is losing / has lost activation)
...
HWND WINAPI GetFocusHook(void) {
    if (g_enabledHooks) {
        ...
        // return NULL (-> calling thread's message queue does not have an associated window with the keyboard focus)
        return NULL;
        // if (g_thisProgramWindow==0) {
...
BOOL WINAPI IsWindowEnabledHook(HWND hWnd) {
    ...
    if (g_enabledHooks) {
        // any IsWindowEnabled check from the target process will return FALSE
        // (not just the windows of that process)
        return FALSE;
    } else {
...
UINT g_subclassRegisteredMsg = RegisterWindowMessage(
    L"Windhawk_SetWindowSubclassFromAnyThread_ignore-focus-gain");
    ...
    switch (uMsg) {
        case WM_KILLFOCUS:
            if(g_enabledHooks && g_thisProgramWindow == hWnd) {
                Wh_Log(L"KILLFOCUS");
                // let KILLFOCUS message propagate as usual
            }
            break;
        case WM_SETFOCUS:
            if (g_enabledHooks && g_thisProgramWindow == 0) {
                setMainWindowThread(hWnd);
                Wh_Log(L"SETFOCUS (set %08X) (ignored)", (DWORD)(ULONG_PTR)hWnd);
            } else {
                Wh_Log(L"SETFOCUS (ignored)");
            }
            return 0;
            // break;
        case WM_ACTIVATEAPP: // Changed windows to/from different application
            if(wParam == TRUE && g_enabledHooks) return 0; // Activation: ignore
            break;
        case WM_ACTIVATE: // Changed windows within same application
            if (((wParam & 0xffff) == WA_ACTIVE) && g_enabledHooks) return 0; //ignore
            break;
